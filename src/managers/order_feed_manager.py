...
class OrderFeedManager:
    ...
    async def _process_order(self, slot):
        ...
        for store in self._registry.all():
            ...
            if order_id == trade_id or parent_id == trade_id:
                await self._handle(store, slot, trade, trade_id)
                return


    async def _handle(self, store: ITradeStore, slot, trade, trade_id: str):
        ...
        if order_id == trade_id:
            if status == 2:
                ...
                self._log.info(f"Parent filled | {order_id}")
                store.update(trade_id, symbol=symbol, qty=qty, entry_price=traded_price)
                store.update(trade_id, trailing_levels=self._calc_trailing(traded_price))
        ...
        if parent_id == trade_id:
            if status == 6 and order_type == 4:
                ...
                if order_id != trade['stop_order_id'].tobytes().rstrip(b'\x00').decode():
                    self._log.info(f"Child SL update")
